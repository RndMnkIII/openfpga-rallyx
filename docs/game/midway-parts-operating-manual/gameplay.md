...
same stick — see [cabinet-io.md](cabinet-io.md).

## The pieces

- **"My Car"** is blue and is moved up, down, left and right by the control lever.
- **Enemy cars are red** and pursue.
- The **smoke screen** is used on the pursuing red cars. It is the single button.
- **Barriers** are static obstacles.
- A **radar screen** shows the check points and the chasers.

## Check points and scoring

There are **10 check points per round**, one of which is the **Special Check
Point**. Points are awarded on a rising ladder, by the order in which they are
...
An **accident** — hitting a barrier or a red car — resets the ladder:

> When an accident occurs (hitting a barrier or red car) you must start over from
> the first 100 points.

Note what this means precisely: the *ladder position* resets, not the score. A
player who crashes after the 7th check point goes back to earning 100 for the
next one.
...
The remaining-fuel bonus is paid **when all check points are cleared**, at 100
points per litre. At 10 litres the fuel indicator turns red and a warning sounds
— that warning is the sound bound to P1 up in the switch and sound test, see
...
- Challenging Stages are rounds **3, 7, 11, 15, …** — every fourth round starting
  at 3.
- **8 red cars** appear, immobile.
- They stay immobile **until My Car's fuel runs out**. Fuel is the timer for the
  stage.
- My Car runs at **full speed** for the duration, which is why the fuel goes
  quickly. The player does not control the throttle.
- After the first Challenging Stage ends, the **scene changes to city streets**
  from the next round on.

That last point describes a persistent change triggered by clearing a stage, not
a property of the stage itself: the scenery stays changed from the next round on.
The manual does not say whether it changes again after later Challenging Stages.
...
Fourteen distinct sounds plus BGM, each of which the test mode can trigger on
demand from a single button press.
...
holds because the sound names on page 7 are themselves phrased as game events
("sound made when check pt cleared"). Two entries have no counterpart on page 9
— "service sound" and the high score noise — because page 9 describes neither
the service switch nor the high score table.
