...
separate instruction card in back box area" — page 8 is that card.

## SW1, SW2 — Method of play
...
## SW3, SW4, SW5 — Difficulty and number of cars per credit
...
Difficulty and starting car count are one setting, not two. There is no way to
select 1 car at LOW difficulty or 3 cars at LOW with anything but the two listed
combinations — the eight rows are the whole option space.
...
> [!NOTE]
> `SW6 = ON, SW7 = ON` is not printed on the card. Three of the four
> combinations are listed and the fourth is simply absent. It is most likely "no
> bonus car", which is the setting other sources describe Rally-X as having, but
> **this manual does not say so** and it is left unstated here rather than filled
> in from elsewhere.
