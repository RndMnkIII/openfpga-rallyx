...
> and no signal name anywhere in the document — it is a component placement map
> and a bill-of-materials legend. The words "Schematics" and "Earlier Style" come
> from the filename, which is an archivist's label; neither phrase appears on
> either sheet.
...
Franklin Pk., Ill. title block. The revisions block on each sheet is **empty** —
...
- **Which positions accept alternate parts**, and what the alternates are — both
  the RAM/latch substitutions on Board I and the custom-chip plug-in replacements
  on both boards.
...
Positions are a column number and a row letter, e.g. `9R`, `1B`, `6P`. Board I
uses columns 7 to 12; Board II uses columns 1 to 6.

Designators are normally printed to the left of their box. Where a resistor or
capacitor occupies that space the drawing **omits the designator entirely** —
four boxes across the two sheets are affected. Positions for those are assigned
by alignment with row and column neighbours and are marked `†` in the grids.
One further box carries a designator that does not parse against its own sheet's
column range; it is discussed at length in
[logic-board-i.md](logic-board-i.md#the-12p-label).

Row letters run `A B C D E F H J K L M N P R` — **`G`, `I`, `O` and `Q` are
skipped**, the usual convention for avoiding characters that read as digits.
Rows are lettered bottom-to-top on both sheets, so `R` is the top row.

## Reading these notes

Both sheets are scans without a text layer; the grids below were read off
rendered page images at up to 4.5x. Positions and part numbers were transcribed
box by box.

Both grids have been read at zoom in full. Every position in both tables was
verified against a crop rendered at 4.2x to 6x — columns 7 to 12 on Board I and 1
to 6 on Board II, top row to bottom, including the long RAM columns and the
sparse bottom rows. No position is carried over from the low-resolution
full-sheet render.
...
counts match the number of positions marked with the corresponding symbol —
six on Board I, twenty-two on Board II.
...
Sparse or ambiguous areas at the sheet margins — a few discrete components,
connector designations and hand-added marks — are called out as untranscribed
rather than guessed.
