...
they are marked `*` and `**` and resolved by the legend below. That is the whole
point of those two notes: the board has more footprints than it has parts.
...
Six RAMs, but **twelve footprints**. Column 9 and column 10 are alternates for
each other, not two banks:

- `MBM2147H` (primary) or `MBM2147E` (substitute) populate **column 9**.
- `93415` (substitute) populates **column 10**.

A board is fitted one way or the other. Reading the grid as twelve RAMs would
double the board's actual memory.
...
The gaps — no `RXI-2`, `-4` or `-5` — are filled on Board II, which carries those
three. The seven `RXI-` parts are numbered across the pair, not per board.
...
it. This is the only place in the document that locates either.
...
resistor `R38/1 K` occupies that space. A label reading **`2P`** is printed to
the *right* of the box instead. At 16x the `2P` is clean, with clear whitespace
on both sides and no trace of a leading digit.
...
`C27` to its left and is labelled **`12 A`** on the right — printed with both
digits. So the sheet does displace a designator when passives crowd it, and does
keep the full column number when it does.

**The reading this leaves.** A dropped `1`. The `12 A` precedent shows the sheet's
own convention would have produced `12P` here, which makes the missing digit the
economical explanation. It remains an inference — the scan shows `2P` and nothing
else — but it is an inference the sheet supports rather than one imposed on it.
...
row and column neighbours**, which is unambiguous — the boxes sit on the same
gridlines as everything else — but is inference rather than transcription. It is
flagged wherever it occurs.
...
edge — a switch of some kind, unnamed and unnumbered. It is distinct from the
`8 POS. DIP. S.W.` at `11E`.
