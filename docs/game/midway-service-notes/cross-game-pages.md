...
> [!IMPORTANT]
> Do not assume a shared page is the same printing in both volumes. Printed page
> 1 demonstrably is not — see [README.md](README.md). Pages 2, 3–6 and 7 below
> were read from **notes1** only; their notes2 counterparts were not opened and
> may be earlier revisions listing fewer games.
...
and `SELF TEST SWITCH`. **The two volumes carry different revisions of this
page** — see [README.md](README.md). Rally-X appears in both:
...
Fox. Earlier machines each name a specific dip switch and position instead, e.g.
Boot Hill `Switch #5 off`, Laguna Racer `Switch #7 off & #8 on`. The page
therefore records a generational change in how Midway exposed the self test, from
a dip switch position to a dedicated slide switch.

> [!NOTE]
> That change is not clean. **Space Encounters sits among the slide-switch
> machines with a dip-switch entry** — `Switch #5 off & #6 on` — in both
> revisions, breaking the run between Rally X and Space Zap. Whatever ordering
> the table follows, it is not strictly chronological by test method.
...
door tilt switch is for on any page examined. It is written as a blanket
instruction under the whole table rather than against any particular game, so
whether it applies to the slide-switch machines, the dip-switch machines, or both
is not stated.
...
settle it — `4K (3604)`, `8K (2708)`, `16K (2716)`. A 2716 is 2K x 8, which is
16K bits; a 2708 is 1K x 8, which is 8K bits. Read that way, Rally-X's entry is
five 32K-bit devices, i.e. five of 4K x 8.

> [!NOTE]
> That reading is derived from the first table's part-number annotations. The
> second table, the one containing Rally-X, carries no part numbers at all —
> only counts — so nothing on the Rally-X row itself states a device type.
...
order and are not sequential by date — Galaga's `508` sits below Gun Fight's
`597` from six years earlier.
...
> [!NOTE]
> The table dates Rally-X to 1981. It is a list of Midway game numbers rather
> than a release chronology, and the column is headed only `DATE` — it does not
> say whether that is a design, release or catalogue date.
...
Galaga is listed with the **same** supply number. Several other supplies are
shared across two or three games — Space Zap, Gorf and Wizard of Wor all take
...
number — the only place on any page examined that names a processor generation
as a grouping.
...
6800, Fairchild F-8 and Zilog. Note the page's own heading is
...
MANUFACTURERS`, `VIDEO GAME MONITORS`, `CARD RACK SYSTEM TEST CARDS` — but were
not opened.
