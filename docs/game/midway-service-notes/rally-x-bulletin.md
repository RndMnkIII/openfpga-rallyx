...
RAM test. Only points 1 and 2 are new: the manual's table is wrong, and this one
replaces it.
...
The bulletin says the **positions** are incorrect. Comparing its table against
the one it supersedes, the positions are not what changed.

Both tables list the same twelve board positions in the same order:
`6C 6A 6D 6B 6K 6J 6L 6H 6M 6F 6N 6E`. What differs is the **message numbering**.
The manual numbers the RAM banks `1` to `6`; this bulletin numbers them `0` to
`5`. Every row is therefore shifted by one bank number against the manual, while
