...
- `sim/` - Icarus Verilog testbenches, one per module, asserting the timing against the Namco board's numbers. `check.vh` holds the shared assertions; `stubs.v` supplies an inert `T80s` because no VHDL simulator is installed. Read `sim/README.md` before adding one - it covers the uninitialized dividers every testbench has to force, and what is deliberately not tested. Run them all with `python tools/sim.py`, or one with `--test <name>`. Never listed in `ap_core.qsf` - simulation only.
